...
suite('Functional Tests', () => {
    ...
    test('Translation with empty tiext', function(done) {
        chai.request(server).post('/api/translate')
    ...
    test('Translation with text that needs no translation', function(done) {
        chai.request(server).post('/api/translate')
            .send({ text: 'This sentence requires no translation.', locale: 'american-to-british' })
            .end(function(err, res) {
                assert.equal(res.status, 200);
                assert.isObject(res.body, 'Response should be an object');
                assert.property(res.body, 'text', 'Response should contain original text');
                assert.property(res.body, 'translation', 'Response should contain translation');
                assert.equal(res.body.text, 'This sentence requires no translation.');
